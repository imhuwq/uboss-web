namespace :city_manager do
  ...
  task :migrate => :environment  do
    ...
    FIFTH_LINE_CITYS  = %w(阜新  葫芦岛 辽源  白山  白城 延边  鸡西 鹤岗 双鸭山 阿勒泰 伊春 佳木斯 七台河 黑河 大兴安岭 乌海  巴彦淖尔 乌兰察布 莱芜 昌都 兴安盟 忻州 阳泉 聊城 滨州 舟山 淮南 淮北 铜陵 黄山 迪庆 锡林郭勒盟 阿拉善盟  巢湖 亳州 池州 宣城 景德镇 萍乡 新余 鹰潭 抚州 鹤壁 漯河 南阳 济源  鄂州 咸宁 随州 恩施  仙桃 潜江 天门  常德 湘西  梅州  汕尾  河源  潮州 云浮 那曲 梧州 北海 防城港 钦州  贵港  百色   贺州  河池 来宾   崇左 三沙 儋州 铜川  汉中  安康  商洛  嘉峪关 金昌 白银 日喀则 天水 武威 张掖 平凉 酒泉 庆阳 定西 陇南 临夏 甘南 石嘴山 吴忠 固原  中卫  海东  海北  黄南 海南 果洛 玉树 海西  广元 遂宁 广安 眉山 雅安 巴中  阿坝  甘孜 凉山 六盘水 安顺  铜仁 黔西南  黔东南 黔南  保山  昭通 普洱 山南 临沧  文山 红河 楚雄 德宏 怒江 阿里 林芝  克拉玛依  吐鲁番 哈密 昌吉  博尔塔拉蒙古  巴音郭楞蒙古 和田 阿克苏 喀什 克孜勒苏柯尔克孜 伊犁哈萨克  塔城)
    ...
    ChinaCity.provinces.each do |province_name, province_code|
      ...
      ChinaCity.list(province_code).each do |city_name, city_code|
        if %w(北京 上海 重庆 天津).any? {|name| match?([name], province_name) }
          city_name, city_code = province_name, province_code
        end
        scope = case
        ...
        scope && scope.find_or_initialize_by(city: city_code).update(rate: 0.5)
      end
    end

    if missings = (ALL_OF_CITYS - matched.to_a)
