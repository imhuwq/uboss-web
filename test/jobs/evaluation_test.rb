...
class EvaluationTest < ActiveJob::TestCase
  describe 'Create Evaluation' do
    ...
    it "should have match attrs" do
      ...
      assert_equal "100.00%" , Evaluation.product_good_reputation_rage(product.id)
      assert_equal "100.00%" , Evaluation.sharer_good_reputation_rage(level2_node.user)
      order2 = create(:order,
                        user: buyer,
                        order_items_attributes: [{
                          product: product,
      ...
      assert_equal "50.00%" , Evaluation.product_good_reputation_rage(product)
      assert_equal "50.00%" , Evaluation.sharer_good_reputation_rage(level2_node.user)
    end
  end
end