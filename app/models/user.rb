class User < ActiveRecord::Base
  ...
  attr_writer :login_identifier

  def login_identifier
    @login_identifier || login || email
  end
  ...
  class << self
    ...
    def find_or_update_by_wechat_oauth(oauth_info)
      ...
      user.update_with_wechat_oauth(oauth_info) if user.present?
      user
    end
  ...
  def can_rebind_agent? # 检查绑定条件
    if !agent.present? # 如果没有绑定,许可
      return true
    else
      if agent.is_super_admin? && !self.authenticated? # 如果绑定对象是超级管理员,且还没有通过认证,许可
        return true
      else # 其他情况不能更换绑定
        errors[:base] << '操作失败，已绑定非官方创客，或已认证'
        return false
      end
    end
  end
  ...
  def bind_seller(seller)
    seller.agent = self
    seller.admin = true
    seller.user_roles << UserRole.seller unless seller.is_seller?
    seller.save
  end
  ...
  def update_with_wechat_oauth(oauth_info)
    tap do
      ...
      self.remote_avatar_url = nil if avatar_url.present?
      save
    end
  end
  ...
  def seller_today_joins
    User.where('agent_id = ? and created_at > ? and created_at < ?', id, Time.now.beginning_of_day, Time.now.end_of_day)
  end
  ...
  def generate_agent_code
    loop do
      self.agent_code = rand(9999..100_000).to_s.ljust(6, '0')
      break unless User.find_by(agent_code: agent_code)
    end
    save
  end
  ...
  def set_wechat_data(data)
    ...
    self.remote_avatar_url = data['headimgurl'] if avatar_identifier.blank?
  end
  ...
  def find_or_create_rongcloud_token
    ...
    user.user_id = id
    user.name = identify
    user.portrait_uri = avatar.url(:thumb)
    user.get_token
    update_columns(rongcloud_token: user.token)
    user.token
  end
  ...
  def ensure_privilege_rate
    self.privilege_rate = privilege_rate.to_i
  end
  ...
  def set_mobile
    self.mobile ||= login unless need_set_login?
  end

  def set_default_role
    unless self.is_agent?
      self.admin = true
      user_roles << UserRole.agent
    end
  end

  def set_service_rate
    user_info.service_rate = 5 if self.agent_id_changed? && user_info.present?
  end
end
