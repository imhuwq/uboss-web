class VerifyCode < ActiveRecord::Base
  ...
  scope :today, ->(user) { where(verified: true, service_product_id: user.service_product_ids).where('updated_at BETWEEN ? AND ?', Time.now.beginning_of_day, Time.now.end_of_day) }
