class CategoriesController < ApplicationController
  ...
  def show
    @categories = Category.electricity_categories.where(use_in_store: true, user_id: @seller.id).order('use_in_store_at').where('id <> ?', params[:id])
    @category = @store.categories.electricity_categories.find(params[:id])
